...
from olim.celery_app import app, get_flask_app
...
def train_ml_model(
    self: Task, model_id: int, user_id: int, force_retrain: bool = False, **kwargs
) -> dict:
    ...
    Args:
        ...
        **kwargs: Additional tracking parameters (description, track_progress, etc.)
...
def batch_predict(
    model_id: int, texts: list[str], version_id: int | None = None, **kwargs
) -> list[dict]:
...
def cleanup_old_versions(model_id: int, keep_last_n: int = 5, **kwargs) -> dict:
...
def clear_prediction_cache(version_id: int | None = None, **kwargs) -> dict:
