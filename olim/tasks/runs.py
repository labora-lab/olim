...
from olim.pipelines import BlockContext, get_store, runner_for
from olim.repositories import PipelineRunRepository


@shared_task(name="run_block")
def run_block(run_id: int, position: int) -> None:
    """Execute one block of a run: load its snapshot, run its (stub) runner over
    the upstream artifact, persist the result. Re-raises on failure so the chain
