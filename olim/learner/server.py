...
from public_api import ActiveLearningBackend, EntryId, Labelling, SlotSet
...
precomputed_unlabelled_dataset_keys: SlotSet[EntryId] = SlotSet(unlabelled_dataset.keys())
...
async def active_learning_backend(concept: str) -> ActiveLearningBackend:
    ...
    try:
        ...
        concept_state_path_tmp = SERVER_STATE_DIR / "concepts" / (mapping[concept] + ".tmp")
...
def new_concept(
    ...
    *,
    n_kickstart: int = 10,
    body: NewConceptUnguided | NewConceptKeywordGuided = Body(..., discriminator="method"),
):
    with master_lock:
...
async def peek_evaluation(concept: str, *, alpha: float = 0.1):
    ...
    async with active_learning_backend(concept) as backend:
        return {
            ...
            "precision_dunno": backend.peek_precision(target=Labelling.DUNNO, alpha=alpha),
            ...
            "auc_dunno": backend.peek_auc_roc_single(target=Labelling.DUNNO, alpha=alpha),
