...
class EpsilonGreedy(BanditExplorer):
    ...
    def inform(self, lever: int, reward: float) -> None:
        self._highest_known_payouts[lever] = max(self._highest_known_payouts[lever], reward)
    ...
    def select_lever(self) -> int:
        if self.rng.random() <= self.epsilon:
            ...
            return self.rng.choice(np.flatnonzero(self._highest_known_payouts == max_value))
