...
from ..settings import RANDOM_SEED, WORK_FOLDER
...
def get_data(project_id: int) -> ...:
    work_folder = get_work_folder().joinpath(str(project_id))
    with work_folder.joinpath("data.json").open("r") as f:
        data = json.load(f)
    return data
...
def get_label_dir(project_id: int, label_id: int) -> Path:
    ...
    label_dir = work_folder.joinpath("active-learning", str(project_id), str(label_id))
    print(label_dir)
